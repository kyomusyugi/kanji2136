...
export default function QuizPage({
  ...
  const kunInputRef = useRef<HTMLInputElement>(null);
  ...
  const handleSubmit = () => {
    ...
    const kunMatch = userKunAnswers.some((ans) =>
      correctKunAnswers.includes(ans)
    );

    const userOnAnswers = normalize(userOn);
    const correctOnAnswers = normalize(currentWord.on);

    const onMatch = userOnAnswers.some((ans) =>
      correctOnAnswers.includes(ans)
    );
